...
- **Finite counters must be explicit in TLC configs.** `MaxPrivActionAttempts=3` keeps the model finite while still exercising both rejected and accepted privileged-action attempts.
